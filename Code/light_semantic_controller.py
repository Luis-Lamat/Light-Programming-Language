...
def add_var_dic(var_name, var_type, var_value,var_function):
	if var_function == 'light':
		if !var_exists_light(var_name):
			var_dict['light'][var_name] = Var(gl_info.get_next_var_id, var_name,'global', type_dict[var_type], var_value)
			return True
		else:
			return False
	else:
		if !var_exists_func(var_name, var_function):
			var_dict['global'][var_function][var_name] = Var(gl_info.get_next_var_id, var_name,'local', type_dict[var_type], var_value)
